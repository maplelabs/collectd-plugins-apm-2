...
def get_job_info(job_id, wfName, wfId, wfaId, wfaName):
    ...
    job_info['time'] = int(round(time.time() * 1000))
...
def get_job_counters(job_id):
    ...
    job_counters_json['time'] = int(round(time.time() * 1000))
...
def get_app_info(app_id, wfName, wfId, wfaId, wfaName):
    ...
    app_info['time'] = int(round(time.time() * 1000))
    ...
    try:
        ...
        for task in task_ids:
            ...
            for task_attempt in task_attempt_list:
                ...
                task_attempt['time'] = int(round(time.time() * 1000))
...
def get_app_details(app_details):
    ...
    for attempt in app_details['attempts']:
        ...
        attempt['time'] = int(round(time.time() * 1000))
...
def get_job_details(app, name, attempt_id):
    ...
    for job in job_details:
        ...
        job['time'] = int(round(time.time() * 1000))
...
def get_executors(app, name, attempt_id):
    ...
    def update(e):
        ...
        e['time'] = int(round(time.time() * 1000))
        e['appName'] = name
...
def get_stages(app, name, attempt_id):
    ...
    def updates(d):
        ...
        d['time'] = int(round(time.time() * 1000))
