...
sys.path.append(path.dirname(path.abspath("/opt/sfapm/collectd/plugins/sf-plugins-hadoop/Collectors/configuration.py")))
sys.path.append(path.dirname(path.abspath("/opt/sfapm/collectd/plugins/sf-plugins-hadoop/Collectors/library/elastic.py")))
sys.path.append(path.dirname(path.abspath("/opt/sfapm/collectd/plugins/sf-plugins-hadoop/Collectors/OozieJobsCollector/processOzzieWorkflows.py")))
sys.path.append(path.dirname(path.abspath("/opt/sfapm/collectd/plugins/sf-plugins-hadoop/Collectors/OozieJobsCollector/processElasticWorkflows.py")))
sys.path.append(path.dirname(path.abspath("/opt/sfapm/collectd/plugins/sf-plugins-hadoop/Collectors/requirements.txt")))
...
#        self.url_knox = "https://localhost:8443/gateway/default/ambari/api/v1/clusters"
    ...
    def update_config_file(self, use_rest_api, jobhistory_copy_dir):
        file_name = "/opt/sfapm/collectd/plugins/sf-plugins-hadoop/Collectors/configuration.py"
        lines = []
        flag = 0
    ...
    def get_app_name(self):
        try:
            with open("/opt/sfapm/collectd/conf/filters.conf", "r") as file_obj:
                for line in file_obj.readlines():
                    if 'MetaData "_tag_appName"' not in line:
                        continue
                    return line.split(" ")[2].strip('"')
        except IOError:
            collectd.error("Could not read file: /opt/sfapm/collectd/conf/filters.conf")

    def get_elastic_search_details(self):
        try:
            with open("/opt/sfapm/collectd/conf/elasticsearch.conf", "r") as file_obj:
                for line in file_obj.readlines():
                    if "URL" not in line:
                        continue
            ...
            collectd.error("Could not read file: /opt/sfapm/collectd/conf/elasticsearch.conf")
    ...
    def collect_data(self):
        ...
        docs = [{"wfId": 0, "wfaId": 0, "wfName": 0, "wfaName": 0, "time": int(math.floor(time.time() * 1000)), "jobId": 0, 'timePeriodStart': 0, 'timePeriodEnd': 0, "mapTaskCount": 0, "reduceTaskCount": 0, 'duration': 0, "_plugin": plugin_name['oozie'], "_documentType": "taskCounts","_tag_appName": tag_app_name['oozie']}]
        for doc in docs:
